...
define Device/emplus_dam-ap410
  DEVICE_VENDOR := Emplus
  DEVICE_MODEL := DAM-AP410
  DEVICE_IMG_PREFIX := $(IMG_PREFIX)-emplus_dam-ap410-signed
  FIT_KEY_DIR := $(TOPDIR)/keys/mtk-secure-boot
  FIT_KEY_NAME := fit_key
  FIT_KEY_ALG := sha256,rsa2048
  DEVICE_DTS := mt7981b-emplus-dam-ap410
  DEVICE_DTS_DIR := ../dts
  DEVICE_DTC_FLAGS := --pad 4096
  DEVICE_DTS_LOADADDR := 0x43f00000
  DEVICE_PACKAGES := kmod-mt7915e kmod-mt7981-firmware mt7981-wo-firmware kmod-mtk-efuse-nl-drv mtk-efuse-nl-tool-mt7981
  KERNEL_LOADADDR := 0x44000000
  KERNEL := kernel-bin | gzip
  KERNEL_INITRAMFS := kernel-bin | lzma | \
        fit lzma $$(KDIR)/image-$$(firstword $$(DEVICE_DTS)).dtb with-initrd | pad-to 64k
  ...
  UBOOTENV_IN_UBI := 1
  IMAGES := sysupgrade.itb
  IMAGE_SIZE := $$(shell expr 96 + $$(CONFIG_TARGET_ROOTFS_PARTSIZE))m
  IMAGE/sysupgrade.itb := append-kernel | fit gzip $$(KDIR)/image-$$(firstword $$(DEVICE_DTS)).dtb external-with-rootfs | pad-rootfs | append-metadata
  ARTIFACTS := spim-nand-preloader.bin spim-nand-bl31-uboot.fip bl2.img.signkeyhash
  ARTIFACT/spim-nand-preloader.bin := mt7981-bl2 dam-ap410-ddr4
  ARTIFACT/spim-nand-bl31-uboot.fip := mt7981-bl31-uboot emplus_dam-ap410
  ARTIFACT/bl2.img.signkeyhash := copy-file $(STAGING_DIR_IMAGE)/mt7981-dam-ap410-ddr4-bl2.img.signkeyhash
endef
TARGET_DEVICES += emplus_dam-ap410

define Device/emplus_dam-ap410_unsigned
  DEVICE_VENDOR := Emplus
  DEVICE_MODEL := DAM-AP410 (unsigned)
  FIT_KEY_DIR :=
  FIT_KEY_NAME :=
  FIT_KEY_ALG :=
  DEVICE_DTS := mt7981b-emplus-dam-ap410
  ...
  IMAGE/sysupgrade.itb := append-kernel | fit gzip $$(KDIR)/image-$$(firstword $$(DEVICE_DTS)).dtb external-with-rootfs | pad-rootfs | append-metadata
  ARTIFACTS := spim-nand-preloader.bin spim-nand-bl31-uboot.fip
  ARTIFACT/spim-nand-preloader.bin := mt7981-bl2 spim-nand-ddr4
